...
class GruposController extends Controller
{
    ...
    public function delete($id)
    {
        ...
        try {
            ...
            $grupoData = $db->query($sql, [':id' => $id]);
            
            $sql = "UPDATE grupos SET activo = false WHERE id = :id";
            $db->query($sql, [':id' => $id]);
            
            // Registrar actividad
            ActivityLogger::logDelete('grupos', $id, $grupoData);
    ...
    public function horarios($id)
    {
        ...
        $grupo = $this->getGrupo($id);
        if (!$grupo) {
            return $this->view->renderWithLayout('errors/404', ['title' => 'Grupo no encontrado']);
        }

        $data = [
            'title' => 'Horarios del Grupo: ' . $grupo['numero'],
            'user' => $this->getCurrentUser(),
            'grupo' => $grupo,
            'horarios' => $this->getHorariosByGrupo($id)
        ];

        return $this->view->renderWithLayout('grupos/horarios', $data);
    }
    
    private function getHorariosByGrupo($grupoId)
    {
        try {
            $db = Database::getInstance();
            return $db->query("SELECT h.*, a.nombre as aula_nombre, a.codigo as aula_codigo
                              FROM horarios h
                              LEFT JOIN aulas a ON h.aula_id = a.id
                              WHERE h.grupo_id = :grupo_id
                              ORDER BY h.dia_semana, h.hora_inicio", [':grupo_id' => $grupoId]);
        } catch (Exception $e) {
            return [];
        }
    }
